...
class RBPTTIterator(Iterator):
    ...
    def __init__(self, dataset, batch_size, bptt_len, **kwargs):
        ...
        if 'bow' in dataset.fields.keys():
            semantic_key = 'bow'
        self.__bptt_2_minibatch = getattr(self, f'_{semantic_key}_bptt_2_minibatches')
        ...
        super(RBPTTIterator, self).__init__(dataset, batch_size, **kwargs)
    ...
    def _bow_bptt_2_minibatches(self, batch, batch_size, dataset):
        time, time_len, bow, ratings, points = self.__bow_series_2_bptt(batch, batch_size)
        yield (Batch.fromvars(dataset, batch_size,
                              ...
                              target_time=t[:, :, -1],
                              target_bow=b[:, :, -1])
               for t, l, b, r, p in zip(time, time_len, bow, ratings, points))
    ...
    def __bow_series_2_bptt(self, batch: Batch, batch_size: int):
        ...
        ratings = ratings.view(batch_size, num_windows, self.bptt_len, -1)
        ratings = ratings.unbind(1)
        points = batch.points

        points = points.view(batch_size, num_windows, self.bptt_len, -1)
        points = points.unbind(1)
        return time, seq_len, bow, ratings, points
