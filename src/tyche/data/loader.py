...
class SimpleDataLoader(object):
    def __init__(self, **kwargs):
        ...
        bptt_size = kwargs.pop('bptt_size', 50)
        seq_length = kwargs.pop('seq_length')
        n_steps_ahead = kwargs.pop('n_steps_ahead', 1)

        x, y = self.load_and_split(data_path, seq_length, n_steps_ahead)
        x_train, x_test_in_batch, x_test_in_time, \
        y_train, y_test_in_batch, y_test_in_time = self.__preprocess(x, y, test_size, bptt_size)
        train_dataset = TensorDataset(torch.tensor(x_train), torch.tensor(y_train))

        test_batch_dataset = TensorDataset(torch.tensor(x_test_in_batch), torch.tensor(y_test_in_batch))
        test_time_dataset = TensorDataset(torch.tensor(x_test_in_time), torch.tensor(y_test_in_time))
        self.__train_data_loader = DataLoader(train_dataset, **kwargs)
        self.__test_batch_data_loader = DataLoader(test_batch_dataset, **kwargs)
        self.__test_time_data_loader = DataLoader(test_time_dataset, batch_size=x_test_in_time.shape[0])
    ...
    @property
    def test_batch_data_loader(self):
        return self.__test_batch_data_loader

    @property
    def test_time_data_loader(self):
        return self.__test_time_data_loader

    def load_and_split(self, data_path: str, max_seq_len: int, n_steps_ahead: int) -> Tuple[np.ndarray, np.ndarray]:
        raw_data = np.load(data_path)[:,
                   :max_seq_len + 1 + n_steps_ahead]  # the length of a time series should be exactly $max_seq_len
        velo = np.diff(raw_data[:, :, :2], axis=1)
        raw_data = np.concatenate((raw_data[:, :-1, :2], velo), axis=-1)
        x = raw_data[:, :-n_steps_ahead]

        y = np.stack([raw_data[:, i:i + n_steps_ahead] for i in range(x.shape[1])], axis=1)  # [ B, T, NSP, D]
        return x, y

    def __preprocess(self, x, y, test_size, bptt_len):
        B, T, D = x.shape
        n_bptt_windows = int(T / bptt_len)
        N = np.int(B * (1. - test_size))
        x_train, x_test_in_batch, x_test_in_time = x[:N, :-bptt_len], x[N:, :-bptt_len], x[:N, -bptt_len:]
        y_train, y_test_in_batch, y_test_in_time = y[:N, :-bptt_len], y[N:, :-bptt_len], y[:N, -bptt_len:]

        self.__fit_scaler(x_train)
        x_train = self.__standardize(x_train)
        x_test_in_batch = self.__standardize(x_test_in_batch)
        x_test_in_time = self.__standardize(x_test_in_time)
        y_train = self.__standardize(y_train)
        y_test_in_batch = self.__standardize(y_test_in_batch)
        y_test_in_time = self.__standardize(y_test_in_time)

        x_train = np.stack(np.split(x_train, n_bptt_windows - 1, axis=1), 1)
        y_train = np.stack(np.split(y_train, n_bptt_windows - 1, axis=1), 1)
        return x_train, x_test_in_batch, x_test_in_time, y_train, y_test_in_batch, y_test_in_time
...
class DataLoaderYelp(ADataLoader):
    def __init__(self, device, **kwargs):
        ...
        if self.load_split:
            ...
            with open('/opt/mlfta/robinsmodels/yelp/yelp_split_sample.pkl', 'rb') as input:
        ...
        if self.save_split:
            ...
            with open('/opt/mlfta/robinsmodels/yelp/yelp_split_sample.pkl', 'wb') as output:
...
class DataLoaderYahoo(ADataLoader):
    def __init__(self, device, **kwargs):
        ...
        if self.load_split:
            print("loading pickle sample split")
            with open('/opt/mlfta/robinsmodels/yahoo/yahoo_split_sample.pkl', 'rb') as input:
                split = pickle.load(input)
                sub_train_ = split[0]
                sub_valid_ = split[1]
        ...
        if self.save_split:
            ...
            with open('/opt/mlfta/robinsmodels/yahoo/yahoo_split_sample.pkl', 'wb') as output:
                pickle.dump(split, output, pickle.HIGHEST_PROTOCOL)
